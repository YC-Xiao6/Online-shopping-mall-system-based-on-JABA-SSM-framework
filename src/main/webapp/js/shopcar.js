...
function productCount(){
  ...
  //   price=myTableTr[i].getElementsByTagName("td")[6].innerHTML;
...
function total() {
  ...
  for (var i=0;i<oInput.length;i++){
    if(oInput[i].checked==true){
      price=myTableTr[i+2].getElementsByTagName("td")[6].innerHTML;
      total+=Number(price)
    }
  }
  document.getElementById("total").innerHTML=total;
...
function deleteRow(sid){
  $j.ajax({
    url :"/shopcar/deleteCar",
    type : "Post", //以Post方式发送请求
    data : {"sid":sid},// 请求时发送的数据,
    ...
    success: function(success) {
      ...
      alert("加入错误,请先登录");
      // location.href = "${pageContext.request.contextPath}/shopcar.jsp";
